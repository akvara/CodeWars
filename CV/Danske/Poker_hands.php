...
function compare_line($line): int {
    ...
    echo print_pocker($hands[0]) . (compare_hands($hands[0], $hands[1]) > 0 ? ' > ' : ' < ') . print_pocker($hands[1]) . PHP_EOL;
...
function compare_hands(array $hand1, array $hand2): int {
    ...
    $hand1value = poker_rank($hand1);
    $hand2value = poker_rank($hand2);

    if ($hand1value['rank'] > $hand2value['rank']) return 1;
    if ($hand1value['rank'] < $hand2value['rank']) return -1;
    if ($RANKED_FUNCTIONS[$hand1value['rank']] === 'royal_flush') return 0;

    $by_value = by_value($hand1value['result']['value'][0], $hand2value['result']['value'][0]);
    if ($by_value !== 0) return $by_value;

    if (strlen($hand1value['result']['value']) === 2) {
        $by_value = by_value($hand1value['result']['value'][1], $hand2value['result']['value'][1]);
        if ($by_value !== 0) return $by_value;
    }

    return compare_hands($hand1value['result']['remaining'], $hand2value['result']['remaining']);
}
...
/* Returns array withour given values */
...
function pocker_flush(array $hand) {
    ...
    return ['value' => current(array_keys($same)), 'remaining' => []];
}
...
class PokerHandsTestCases extends TestCase
{
    ...
    public function testCases() {
        ...
        $this->assertSame(compare_line("AH TH KH JH QH AS TS KS JS QS"), 0, 'Nope.');
