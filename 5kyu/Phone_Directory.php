...
function phone($strng, $num) {
    ...
    $arr = explode("\n", $strng);
    foreach ($arr as $line) {
        $s = trim(preg_replace("/[^<>_+\- .,0-9A-Za-z']/", "", $line));
        $pos1 = strpos($s, "<");
        $pos2 = strpos($s, ">");
        $name = substr($s, $pos1 + 1, $pos2 - $pos1 - 1);
        $s = substr($s, 0, $pos1) . substr($s, $pos2 + 2);
        $number = cut_number($s);
        $s = str_replace(',', '', $s);
        $s = str_replace('_', ' ', $s);
        $s = trim(str_replace('  ', ' ', $s));
        if (isset($dir[$number]))
            $dir[$number] = 'Error => Too many people: ' . $number;
        else
            $dir[$number] = 'Phone => ' . $number . ', Name => ' .  $name. ', Address => ' . $s;
    }

    if (isset($dir[$num])) return $dir[$num];
    return "Error => Not found: " . $num;
}

function cut_number(&$string) {
    if (preg_match ('/\+[-0-9]+/', $string, $matches)) {
        $res = $matches[0];
        $string = str_replace($res, "", $string);
        return substr($res, 1);
    }
    return '';
}
...
class PhoneDirTestCases extends TestCase {
    ...
    private $num = "1-541-754-3010,1-541-914-3010,1-541-984-3012,1-321-512-2222,1-741-984-3090,"
